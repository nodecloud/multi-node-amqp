...
let createConnections = (() => {
    var _ref = _asyncToGenerator(function* (options) {
        ...
        const promises = opts.map((() => {
            var _ref2 = _asyncToGenerator(function* (option) {
                try {
                    connections[option.id] = yield createConnection(option.id, option.options, option.extra);
                } catch (e) {
                    errors[option.id] = e;
                }
            });
    ...
    return function createConnections(_x) {
...
exports.default = {
    ...
    getQueue(connId, queueName) {
        const conn = connManager.getConnection(connId);
        if (conn) {
            return conn.getQueue(queueName);
        }

        return null;
    },

    getExchange(connId, exchangeName) {
        const conn = connManager.getConnection(connId);
        if (conn) {
            return conn.getExchange(exchangeName);
        }
    ...
    getState() {
        return {
            connections: connManager.getAvailableConnections().map(conn => {
                return conn.getState();
            })
        };
    }
};
function createConnection(id, options, extra) {
    let isInitial = true;
